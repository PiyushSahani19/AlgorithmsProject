...
//	CompanyStocks shareDetails = new CompanyStocks();
	...
	public void displayStocksAscending(CompanyStocks[] companystocks) {
		double[] listOfStockPrice = new double[companystocks.length];
		listOfStockPrice = operations.getAllStockpriceList(companystocks);
		int noOfStocks = listOfStockPrice.length;
		operations.sortStocks(listOfStockPrice, noOfStocks, noOfStocks, true);
	...
	public void displayStocksDescending(CompanyStocks[] companystocks) {
		double[] listOfStockPrice = operations.getAllStockpriceList(companystocks);

		operations.sortStocks(listOfStockPrice, 0, listOfStockPrice.length - 1, false);
		int noOfStocks = listOfStockPrice.length;
		for (int i = 0; i < noOfStocks; ++i)
			System.out.print(listOfStockPrice[i] + " ");
		System.out.println();
